                ...
                <div class="row g-3 align-items-center">
                  ...
                  <div class="col-auto">
                    <select
                      name="language"
                      class="form-select form-select-sm"
                      v-model="languageModel2"
                      @change="onSelectLanguageChange($event)"
                    ...
                    <select
                      name="language"
                      class="form-select form-select-sm"
                      v-model="visibilityModel2"
                      @change="onSelectVisibilityChange($event)"
                  ...
                  <div class="col-auto">
                    <button
                      ...
                      id="fileinput-button"
                      :disabled="!categoryModel2 || !languageModel2 || !visibilityModel2"
                      @click="openBrowse"
                    >
            ...
            <div class="table-responsive  pt-1">
              ...
              <thead>
                ...
                <tr v-if="!loadingSpinner" v-for="(row, index) in galleryDatas" :key="row.id">
                  ...
                  <td align="right">{{ bytesToSize(row.file_size) }} </td>
...
export default {
  ...
  data() {
    return {
      cuVerseUploadUploadUrl: cuVerseUploadUploadUrl,
      cuVerseUploadListUrl: cuVerseUploadListUrl,
      cuVerseUploadDeleteUrl: cuVerseUploadDeleteUrl,
      defaultdictDefaultCategoryMessage: '<p class="text-danger">Please select category.</p>',
      defaultdictDefaultLanguageMessage: '<p class="text-danger">Please select language.</p>',
      defaultdictDefaultVisibilityMessage: '<p class="text-danger">Please select visibility.</p>',
      categoryModel2: "",
      languageModel2: "",
      visibilityModel2: "",
      config: {
        previewTemplate: this.template(),
        url: cuVerseUploadUploadUrl,
        headers: { "My-Awesome-Header": "header value","X-CSRF-TOKEN": window.axios.defaults.headers.common['X-CSRF-TOKEN'] },
        params:{project_id: projectId, language:'', category:'', visibility:''},
        maxFilesize: 1000, // MB
        maxFiles: 50,
        timeout: 0,
        ...
        thumbnailHeight: 50,
        //addRemoveLinks: true,
        acceptedFiles: ".jpg, .jpeg, .png, .pdf, .zip, .mp4, .svg, .docx, .doc",
      ...
      galleryDatas: [],
      categoryOptions: cuVerseCategories,
      languageOptions: cuVerseLanguages,
      visibilityOptions: cuVerseVisibilities,
      showModal: false,
      //isModalOpen: false,
      title: "",
  ...
  watch: {
    modalOptions: function(newVal) {
      ...
      this.categoryModel2 = "";
      this.config.dictDefaultMessage = this.defaultdictDefaultCategoryMessage;
    },
    showModal: function() {
  ...
  mounted() {
    //console.log("gallery upload cuVerseUploadUploadUrl",cuVerseUploadUploadUrl);
    this.categoryModel2 = "";
  ...
  methods: {
    getItems() {
      ...
      this.axios.get(cuVerseUploadListUrl).then(response => {
        setTimeout(() => {
    ...
    deleteItem:function (item) {
      ...
      if(confirm('Are you sure want to delete?')){
        this.loadingSpinner = true;          
        this.axios({method: 'delete', url: cuVerseUploadDeleteUrl+'/'+item.id}).then(response => {
          setTimeout(() => {
            this.loadingSpinner = false;
            this.getItems();
          },1000);
    ...
    onSelectCategoryChange(event) {
      ...
      //console.log("onSelectCategoryChange", event.target.value, this.categoryModel2);
      this.config.params.category = event.target.value; 

     if (!this.categoryModel2) {
        this.config.dictDefaultMessage = this.defaultdictDefaultCategoryMessage;
        this.$refs.galleryUploadDropZone.disable();
      }else if (!this.languageModel2) {
        this.config.dictDefaultMessage = this.defaultdictDefaultLanguageMessage;
        this.$refs.galleryUploadDropZone.disable();
      } else if (!this.visibilityModel2) {
        this.config.dictDefaultMessage = this.defaultdictDefaultVisibilityMessage;
        this.$refs.galleryUploadDropZone.disable();
      } else {
    ...
    onSelectLanguageChange(event) {
      ...
      this.config.params.language = event.target.value; 
      console.log("onSelectLanguageChange",event.target.value);
    ...
    onSelectVisibilityChange(event) {
      if (event) {
        event.preventDefault();
      }
      this.config.params.visibility = event.target.value; 
      console.log("onSelectVisibilityChange",event.target.value);
    ...
    vdropzoneCompleteMultiple(file) {
      console.log("vdropzoneCompleteMultiple", response);
      setTimeout(() => {
        this.categoryModel2 = '';
        this.languageModel2 = '';
        this.visibilityModel2 = '';
        this.config.dictDefaultMessage = this.defaultdictDefaultCategoryMessage;
        this.$refs.galleryUploadDropZone.disable();
      }, 3000);
    },
      ...
      return `
            <div id="previews-gallery" class="file-row">
                <table class="table table-sm">
                    ...
                    <tr>
                        <td>
                            <span class="preview"><img width="100" data-dz-thumbnail/></span>
                        </td>
                        <td>
